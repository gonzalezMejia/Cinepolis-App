...
class ProfilePage extends GetView<ProfileController> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: SimpleAppBar(title: "Mi Perfil",actions: [IconButton(onPressed: ()=>controller.singOut(), icon: const Icon(Icons.exit_to_app))]),
        body: Obx(() {
            ...
            child: Center(
              child: Column(
                children: <Widget>[
                  ...
                  _textPain(context, "Télefono",
                      controller.profile.first.telefono != 0
                          ? controller.profile.first.telefono
                          .toString()
                          : 'Sin Teléfono')
                      .paddingOnly(bottom: 15),
                  _textPain(context, "Correo", controller.profile.first.correoE)
                      .paddingOnly(bottom: 15)
                ],
              ),
            ),
